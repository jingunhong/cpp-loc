...
- Copy implementation, tests, configuration, and documentation from `Cpp-SWE-bench`
  at `f40360c9c49ebf128e75fe92fae73da2fc2291d8` onto the existing initial history of
  `jingunhong/cpp-loc`. Keep the original repository and historical artifacts intact;
...
- Name the Python distribution `cpp-loc` and rename the import package to `cpp_loc`,
  ...
  entries above retain the old package paths. Point future generated dataset cards
...
- Ignore local corpora, clones/caches, release drafts, and dataset shard formats;
  ...
  sibling checkout using explicit paths. No dataset is regenerated by migration,
